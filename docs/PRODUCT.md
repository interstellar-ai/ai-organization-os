...
- The organization researches and selects channels; the Founder handles only identity, legal, payment, credential and final-authority boundaries.
...
- A controlled continuous-improvement loop that deduplicates task, quality, integration, capability and uncertain external-action signals; exposes a bounded backlog to the CEO; and requires Founder-confirmed goals plus verification evidence;
...
The current version uses real model reasoning for CEO conversation and planning, staffing proposals, general document work, independent review, final reporting, and software development. Home is the CEO conversation surface: it supplies a timestamped host-generated operational summary, persists the discussion, and permits a Founder-confirmed goal suggestion, but grants no tools or mutation authority. Founder-only decisions are collected in Decisions, while just-in-time registration actions appear on the affected external task. The complete work proposal and autonomy budget remain in Goals before projects and delivery tasks are created. Approved hires must use existing job templates and trigger replanning; no employee can self-create or bypass access policy. The standalone work-request classifier, improvement observer and initial provider catalog remain deterministic and bounded. Code integration and selected external connectors are host-controlled, approval-gated workflows. Image generation, full telemetry-driven diagnosis, live autonomous provider discovery, direct adapters for every storefront, autonomous job-template creation, standing staffing budgets, durable multi-user storage, authenticated runtime identity, currency budgets, production isolation, general output data-flow controls, distributed workers, project-scoped asset policies, deployment, canary rollback, and remote Git integration remain future work. See [CEO conversation](CEO_CONVERSATION.md), [Goal planning](GOAL_PLANNING.md), [Controlled autonomy](CONTROLLED_AUTONOMY.md), [Controlled continuous improvement](CONTINUOUS_IMPROVEMENT.md), [Code integration](CODE_INTEGRATION.md), [External connectors](EXTERNAL_CONNECTORS.md), and [Durable runtime](DURABLE_RUNTIME.md).
