...
function renderExternalConfiguration(task) {
  ...
  if (founderAction) {
    ...
    const priorRoutes = state.founderActions.filter((item) => item.taskId === task.id && item.status === "unavailable")
      .map((item) => `${escapeHtml(item.recommendation.provider)} (${escapeHtml(titleize(item.routingFeedback?.reason || "unavailable"))})`).join(", ");
...
document.querySelector("#workDeliveryContent").addEventListener("submit", async (event) => {
  const form = event.target.closest("[data-configure-code], [data-configure-external], [data-request-integration], [data-complete-founder-action], [data-founder-action-unavailable], [data-manual-external-result]");
  if (!form) return;
  event.preventDefault();
