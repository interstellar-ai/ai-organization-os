...
function renderExternalConfiguration(task) {
  ...
  if (founderAction) {
    ...
    const sources = (recommendation.sourceUrls || []).map((url, index) => `<a href="${escapeHtml(url)}" target="_blank" rel="noreferrer">Provider reference ${index + 1}</a>`).join(" · ");
    const priorRoutes = state.founderActions.filter((item) => item.taskId === task.id && item.status === "unavailable");
    const priorRouteHistory = priorRoutes.length ? `<div class="route-history"><strong>Previous routes</strong><ul>${priorRoutes.map((item) => {
      const label = `${escapeHtml(item.recommendation.provider)} (${escapeHtml(titleize(item.routingFeedback?.reason || "unavailable"))})`;
      const recover = founderAction.status === "pending" && item.recommendation.provider !== founderAction.recommendation.provider
        ? `<form data-founder-action-recover="${escapeHtml(item.id)}"><button class="text-button" type="submit">Use ${escapeHtml(item.recommendation.provider)} again</button></form>` : "";
      return `<li><span>${label}</span>${recover}</li>`;
    }).join("")}</ul></div>` : "";
    const completion = founderAction.completion;
    const registration = `<form data-complete-founder-action="${escapeHtml(founderAction.id)}"><label>Public store or account URL<input type="url" name="publicAccountUrl" required placeholder="https://…" /></label><label class="plan-consent"><input type="checkbox" name="registrationComplete" required /> Account registration is complete.</label><label class="plan-consent"><input type="checkbox" name="identityAndTermsConfirmed" required /> Required verification and provider terms are complete.</label><label class="plan-consent"><input type="checkbox" name="paymentReady" required /> An eligible payment or delivery account is ready.</label><button class="primary-button" type="submit">Registration completed</button></form>`;
    const routeFeedback = `<details class="route-feedback"><summary>This route is unavailable</summary><p>Record only eligibility constraints. Do not enter an account name, email, password, payment credential or identity document.</p><form data-founder-action-unavailable="${escapeHtml(founderAction.id)}"><label>Why is this route unavailable?<select name="reason" required><option value="">Select a reason</option><option value="payment_account_incompatible">Payment account is incompatible</option><option value="country_unavailable">Provider is unavailable in my country or region</option><option value="verification_unavailable">I cannot complete the required verification</option><option value="terms_unacceptable">Provider terms are not acceptable</option><option value="other">Another eligibility constraint</option></select></label><label>Country or region code, optional<input name="countryCode" maxlength="2" pattern="[A-Za-z]{2}" placeholder="For example: CN" /></label><label>Available account type<select name="accountType" required><option value="">Select an account type</option><option value="personal">Personal only</option><option value="business">Business available</option><option value="none">No eligible account</option></select></label><label>Available payment method<select name="paymentRail" required><option value="">Select a payment method</option><option value="paypal">PayPal</option><option value="stripe">Stripe</option><option value="bank">Bank payout</option><option value="other">Another method</option><option value="none">No supported method</option></select></label><button class="secondary-button" type="submit">Use a different channel</button></form></details>`;
    panel.insertAdjacentHTML("beforeend", `<section class="external-readiness"><div class="goal-top"><div><span class="section-kicker">AI-SELECTED CHANNEL</span><h4>${escapeHtml(recommendation.provider)}</h4></div><span class="status ${escapeHtml(founderAction.status)}">${escapeHtml(titleize(founderAction.status))}</span></div><p>${escapeHtml(recommendation.summary)}</p>${priorRouteHistory}<a class="text-button" href="${escapeHtml(recommendation.providerUrl)}" target="_blank" rel="noreferrer">Open provider registration</a><h4>Why this route</h4><ul>${recommendation.reasons.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul><details><summary>Alternatives considered</summary><ul>${alternatives}</ul></details><div class="responsibility-grid"><div><h4>Founder-only steps</h4><ul>${checklist}</ul></div><div><h4>AI organization continues</h4><ul>${systemSteps}</ul></div></div><p class="notice">${escapeHtml(recommendation.knowledgeStatus)}${sources ? `<br />${sources}` : ""}</p>${founderAction.status === "pending" ? `${registration}${routeFeedback}` : `<div class="action-preview"><strong>Founder setup completed</strong><p>${escapeHtml(completion?.publicAccountUrl || "Public account confirmed")}</p><p>No password, payment credential or API secret is stored in this record.</p></div>`}</section>`);
  }
...
document.querySelector("#workDeliveryContent").addEventListener("submit", async (event) => {
  const form = event.target.closest("[data-configure-code], [data-configure-external], [data-request-integration], [data-complete-founder-action], [data-founder-action-unavailable], [data-founder-action-recover], [data-manual-external-result]");
  if (!form) return;
  event.preventDefault();
  ...
  try {
    ...
    } else if (form.dataset.founderActionUnavailable) {
      const values = Object.fromEntries(new FormData(form));
      const result = await api(`/api/founder-actions/${form.dataset.founderActionUnavailable}/unavailable`, { method: "POST", body: JSON.stringify({
        reason: values.reason,
        countryCode: values.countryCode,
        accountType: values.accountType,
        paymentRail: values.paymentRail
      }) });
      toast(result.replacement
        ? `${result.unavailable.recommendation.provider} marked unavailable. The AI organization selected ${result.replacement.recommendation.provider}.`
        : "Route marked unavailable. The AI CEO must research another provider.");
    } else if (form.dataset.founderActionRecover) {
      const result = await api(`/api/founder-actions/${form.dataset.founderActionRecover}/recover`, { method: "POST", body: "{}" });
      toast(`${result.recovered.recommendation.provider} restored as the active channel. Confirm account readiness to continue.`);
